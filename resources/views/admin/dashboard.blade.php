...
<div class="space-y-6">
    ...
    <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        @foreach ($cards as $index => $card)
            <div class="fade-up rounded-2xl border border-[#d8e0da] bg-white p-5 shadow-[0_1px_2px_rgba(26,31,28,0.04)]" style="animation-delay: {{ $index * 0.06 }}s">
                <div class="flex items-start justify-between">
                    <p class="text-xs font-semibold uppercase tracking-wider text-ink-soft/50">{{ $card['label'] }}</p>
                    <span @class([
                        'flex h-9 w-9 items-center justify-center rounded-xl',
                        'bg-[#e4efe8] text-[#2a4a37]' => $card['tone'] === 'sage',
                        'bg-[#fdf0e8] text-[#c45c26]' => $card['tone'] === 'ember',
                    ])>
                ...
                <div class="mt-2.5 flex items-center gap-1.5 text-sm">
                    @if (! is_null($card['delta']))
                        ...
                        <span @class([
                            ...
                            'bg-[#e4efe8] text-[#2a4a37]' => $up,
                            'bg-[#fdf0e8] text-[#c45c26]' => ! $up,
                        ])>
    ...
    <div class="grid gap-4 xl:grid-cols-5">
        ...
        <section class="fade-up xl:col-span-3 rounded-2xl border border-[#d8e0da] bg-white p-5 shadow-[0_1px_2px_rgba(26,31,28,0.04)] sm:p-6" style="animation-delay: 0.12s">
            <div class="mb-6 flex items-end justify-between gap-4">
                ...
                <div class="text-right">
                    <p class="font-display text-xl font-extrabold text-[#3d6b4f]">Rs {{ number_format($stats['revenue_week']) }}</p>
                ...
                @foreach ($weeklyRevenue as $index => $day)
                    ...
                    <div class="group flex flex-1 flex-col items-center gap-2">
                        <div class="relative flex w-full flex-1 items-end">
                            <div
                                class="bar-fill relative w-full rounded-t-lg bg-gradient-to-t from-[#2a4a37] to-[#5f9873] transition-all duration-300 group-hover:from-[#c45c26] group-hover:to-[#e08a56]"
        ...
        {{-- Top items --}}
        <section class="fade-up xl:col-span-2 rounded-2xl border border-[#d8e0da] bg-white p-5 shadow-[0_1px_2px_rgba(26,31,28,0.04)] sm:p-6" style="animation-delay: 0.18s">
            ...
            <ul class="mt-5 space-y-2.5">
                @forelse ($topItems as $rank => $item)
                    <li class="flex items-center gap-3 rounded-xl border border-transparent bg-[#f6f8f6] px-3 py-2.5 transition hover:border-[#d8e0da]">
                        ...
                        <p class="shrink-0 text-sm font-bold text-[#3d6b4f]">Rs {{ number_format($item['revenue']) }}</p>
                    </li>
                @empty
                    <li class="rounded-xl bg-[#f6f8f6] px-3 py-8 text-center text-sm text-ink-soft/55">No sales recorded yet.</li>
                @endforelse
            </ul>
        </section>
    </div>
    ...
    {{-- Recent orders --}}
    <section class="fade-up overflow-hidden rounded-2xl border border-[#d8e0da] bg-white shadow-[0_1px_2px_rgba(26,31,28,0.04)]" style="animation-delay: 0.24s">
        <div class="flex items-center justify-between border-b border-[#eef2ef] px-5 py-4 sm:px-6">
            <div>
            ...
            <table class="w-full min-w-[640px] text-left text-sm">
                ...
                <tbody class="divide-y divide-[#eef2ef]">
                    @forelse ($recentOrders as $order)
                        <tr class="transition hover:bg-[#f6f8f6]/70">
                            ...
                            <td class="px-5 py-3.5 sm:px-6">
                                @php
                                    $statusClass = match ($order->status) {
                                        'Completed' => 'bg-[#e4efe8] text-[#2a4a37]',
                                        'Preparing' => 'bg-[#fdf0e8] text-[#c45c26]',
