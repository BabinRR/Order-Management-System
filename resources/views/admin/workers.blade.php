...
>
    {{-- Toolbar --}}
    <div class="flex flex-col gap-3 rounded-2xl border border-[#d8e0da] bg-white p-4 shadow-[0_1px_2px_rgba(26,31,28,0.04)] sm:flex-row sm:items-center sm:justify-between">
        <div class="flex items-center gap-3">
            ...
            <span class="hidden rounded-lg bg-[#eef2ef] px-2.5 py-1 text-xs font-semibold text-ink-soft/60 sm:inline-flex">{{ $workers->count() }} total</span>
        </div>
        <button
            type="button"
            @click="openCreate()"
    ...
    {{-- Table --}}
    <div class="overflow-hidden rounded-2xl border border-[#d8e0da] bg-white shadow-[0_1px_2px_rgba(26,31,28,0.04)]">
        <div class="overflow-x-auto">
            <table class="w-full min-w-[760px] text-left text-sm">
                <thead class="bg-[#f6f8f6] text-xs uppercase tracking-wider text-ink-soft/50">
                    <tr>
                ...
                <tbody class="divide-y divide-[#eef2ef]">
                    @forelse ($workers as $worker)
                        <tr
                            class="transition hover:bg-[#f6f8f6]/70"
                            x-show="matches(@js($worker->name.' '.$worker->role.' '.$worker->email.' '.$worker->shift.' '.$worker->status))"
                        >
                            <td class="px-5 py-3.5 sm:px-6">
                                <div class="flex items-center gap-3">
                                    <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-[#e4efe8] to-[#d0e2d7] text-xs font-bold text-[#2a4a37]">
                                        {{ $worker->initials }}
                                    </div>
                                    <div class="min-w-0">
                            ...
                            <td class="px-5 py-3.5 sm:px-6">
                                <span class="inline-flex rounded-md bg-[#eef2ef] px-2 py-0.5 text-xs font-medium text-ink-soft/70">{{ $worker->shift }}</span>
                            </td>
                            <td class="px-5 py-3.5 sm:px-6">
                                @php
                                    $statusClass = match ($worker->status) {
                                        'Active' => 'bg-[#e4efe8] text-[#2a4a37]',
                                        'On Leave' => 'bg-[#fdf0e8] text-[#c45c26]',
            ...
            <form :action="editingId ? baseUrl + '/' + editingId : storeUrl" method="POST" class="space-y-4">
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    <div>
                        ...
                        <select x-model="form.role" name="role" class="w-full rounded-xl border border-[#d8e0da] px-3 py-2.5 text-sm outline-none focus:border-[#3d6b4f] focus:ring-2 focus:ring-[#3d6b4f]/25">
                    ...
                    <div>
                        ...
                        <select x-model="form.shift" name="shift" class="w-full rounded-xl border border-[#d8e0da] px-3 py-2.5 text-sm outline-none focus:border-[#3d6b4f] focus:ring-2 focus:ring-[#3d6b4f]/25">
                ...
                <div>
                    ...
                    <input x-model="form.email" name="email" type="email" @class(['w-full rounded-xl border px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-[#3d6b4f]/25', 'border-[#c45c26]' => $errors->has('email'), 'border-[#d8e0da] focus:border-[#3d6b4f]' => ! $errors->has('email')])>
                    @error('email') <p class="mt-1 text-xs font-medium text-[#c45c26]">{{ $message }}</p> @enderror
                </div>

                <div class="grid gap-4 sm:grid-cols-2">
                    <div>
                        <label class="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-ink-soft/55">Phone</label>
                        <input x-model="form.phone" name="phone" type="text" class="w-full rounded-xl border border-[#d8e0da] px-3 py-2.5 text-sm outline-none focus:border-[#3d6b4f] focus:ring-2 focus:ring-[#3d6b4f]/25">
                    </div>
                    <div>
                ...
                <div class="flex justify-end gap-2 pt-2">
                    ...
                    <button type="submit" class="rounded-xl bg-gradient-to-br from-[#3d6b4f] to-[#2a4a37] px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:opacity-95" x-text="editingId ? 'Save changes' : 'Add worker'"></button>
                </div>
            </form>
        </div>
    </div>
