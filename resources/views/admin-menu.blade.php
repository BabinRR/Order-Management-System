<x-admin-layout title="Food Menu" eyebrow="Catalog" heading="Food Menu">
@php
...
>
    {{-- Toolbar --}}
    <div class="flex flex-col gap-3 rounded-2xl border border-[#d9cbb8] bg-white p-4 shadow-[0_1px_2px_rgba(26,31,28,0.04)] lg:flex-row lg:items-center lg:justify-between">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
            ...
            <select
                ...
                class="rounded-xl border border-[#d9cbb8] bg-[#f7f0e8] px-3 py-2.5 text-sm font-medium outline-none transition focus:border-[#8b5e3c] focus:bg-white focus:ring-2 focus:ring-[#8b5e3c]/25"
            >
    ...
    <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
        @forelse ($items as $item)
            <article
                class="flex flex-col overflow-hidden rounded-2xl border border-[#d9cbb8] bg-white shadow-[0_1px_2px_rgba(26,31,28,0.04)] transition hover:border-[#8b5e3c]/40 hover:shadow-md"
                x-show="matches(@js($item->name), @js($item->category), @js($item->description ?? ''), @js($item->category))"
            >
                @if ($item->image_url)
                    <div class="aspect-[16/10] overflow-hidden bg-[#f0e6da]">
                        <img src="{{ $item->image_url }}" alt="{{ $item->name }}" class="h-full w-full object-cover">
                    </div>
                @else
                    <div class="flex aspect-[16/10] items-center justify-center bg-[#f0e6da] text-ink-soft/35">
                ...
                <div class="flex items-start justify-between gap-3">
                    <div class="min-w-0">
                        <span class="inline-flex rounded-lg bg-[#f0e6da] px-2 py-0.5 text-[11px] font-semibold uppercase tracking-wide text-ink-soft/65">{{ $item->category }}</span>
                        <h3 class="mt-2 font-display text-lg font-extrabold text-ink">{{ $item->name }}</h3>
                    </div>
                    @php
                        $statusClass = match ($item->status) {
                            'Available' => 'bg-[#ede0d0] text-[#5d4037]',
                            'Sold Out' => 'bg-[#f5e6d8] text-[#a0522d]',
                ...
                <div class="mt-4 flex items-center justify-between gap-3 border-t border-[#f0e6da] pt-4">
                    <p class="font-display text-xl font-extrabold text-[#8b5e3c]">Rs {{ number_format($item->price) }}</p>
    ...
    >
        <div @click.outside="modalOpen = false" class="modal-enter w-full max-w-lg rounded-2xl border border-[#d9cbb8] bg-white p-5 shadow-2xl sm:p-6">
            ...
            <form :action="editingId ? baseUrl + '/' + editingId : storeUrl" method="POST" enctype="multipart/form-data" class="space-y-4">
                ...
                <div>
                    <label class="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-ink-soft/55">Photo</label>
                    <input type="file" name="image" accept="image/*" class="w-full rounded-xl border border-[#d9cbb8] bg-[#f7f0e8] px-3 py-2.5 text-sm file:mr-3 file:rounded-lg file:border-0 file:bg-[#8b5e3c] file:px-3 file:py-1.5 file:text-xs file:font-semibold file:text-white">
                    @error('image') <p class="mt-1 text-xs font-medium text-[#a0522d]">{{ $message }}</p> @enderror
                </div>
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    <div>
                        ...
                        <select x-model="form.category" name="category" class="w-full rounded-xl border border-[#d9cbb8] px-3 py-2.5 text-sm outline-none focus:border-[#8b5e3c] focus:ring-2 focus:ring-[#8b5e3c]/25">
                            @foreach ($categories as $category)
                                <option value="{{ $category }}">{{ $category }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input x-model.number="form.price" name="price" type="number" min="0" step="1" @class(['w-full rounded-xl border px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-[#8b5e3c]/25', 'border-[#a0522d]' => $errors->has('price'), 'border-[#d9cbb8] focus:border-[#8b5e3c]' => ! $errors->has('price')])>
                        @error('price') <p class="mt-1 text-xs font-medium text-[#a0522d]">{{ $message }}</p> @enderror
                    </div>
                </div>

                <div>
                    <label class="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-ink-soft/55">Description</label>
                    <textarea x-model="form.description" name="description" rows="3" class="w-full rounded-xl border border-[#d9cbb8] px-3 py-2.5 text-sm outline-none focus:border-[#8b5e3c] focus:ring-2 focus:ring-[#8b5e3c]/25"></textarea>
                    @error('description') <p class="mt-1 text-xs font-medium text-[#a0522d]">{{ $message }}</p> @enderror
                </div>
                ...
                <div class="flex justify-end gap-2 pt-2">
                    ...
                    <button type="submit" class="rounded-xl bg-gradient-to-br from-[#8b5e3c] to-[#5d4037] px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:opacity-95" x-text="editingId ? 'Save changes' : 'Add item'"></button>
                </div>
            </form>
        </div>
    </div>
